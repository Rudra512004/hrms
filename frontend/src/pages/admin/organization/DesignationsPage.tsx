...
import { organizationService, type Designation, type Organization } from '../../../services/organization';
...
export const DesignationsPage: React.FC = () => {
  ...
  const canManage = hasPermission('designation.manage');
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [desigData, orgData] = await Promise.all([
      ...
      if (err.response?.status === 403) {
        setError("403 Forbidden: You do not have permission to view designations.");
      } else {
        setError("Failed to load designations. Backend might be unavailable.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const openEditModal = (desig: Designation) => {
    ...
    setFormData({ 
      ...
      organization: desig.organization,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingDesig) {
        await organizationService.updateDesignation(editingDesig.id, payload);
      } else {
  ...
  const columns = [
    ...
    { key: 'organization', title: 'Organization', render: (n: Designation) => n.organization_name || `Org #${n.organization}` },
    { 
      key: 'is_active', 
      title: 'Status', 
      render: (n: Designation) => (
      ...
      <Card>
        {designations.length === 0 ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <Table data={designations} columns={columns} keyExtractor={(n) => n.id} />
          </div>
        )}
      </Card>
      ...
      {isModalOpen && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            <h2 style={{ marginTop: 0 }}>{editingDesig ? 'Edit Designation' : 'Add Designation'}</h2>
